...
const extractInquiryArray = (
  payload: InquiriesResponseShape,
): AdminInquiryRaw[] => {
...
export const getAdminInquiries = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `문의 목록 조회 실패: ${response.status} ${response.statusText}`,
    )
  }
...
export const answerAdminInquiry = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `문의 답변 등록 실패: ${response.status} ${response.statusText}`,
    )
  }
}
 ...
 */
export const runAggregateSummaryBatch = async (
  targetDate: string,
): Promise<void> => {
  const response = await authenticatedFetch(
    `${API_BASE_URL}/admin/batch/aggregate-summary`,
    {
      method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `배치 실행 실패: ${response.status} ${response.statusText}`,
    )
  }
}
