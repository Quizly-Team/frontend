...
export const getUserInfo = async (): Promise<ReadUserInfoResponse> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `유저 정보 조회 실패: ${response.status} ${response.statusText}`,
    )
  }
 ...
 * 닉네임 변경 응답 타입
 */
export type UpdateUserNickNameResponse = Record<string, never>
...
export const updateNickname = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `닉네임 변경 실패: ${response.status} ${response.statusText}`,
    )
  }
...
export const updateProfileImage = async (
  ...
  const response = await authenticatedFetch(
    `${API_BASE_URL}/account/profileImage`,
    {
      method: 'PUT',
      body: formData,
    },
...
export const getTodaySummary = async (): Promise<{
  ...
  const response = await authenticatedFetch(
    `${API_BASE_URL}/account/today-summary`,
    {
      method: 'GET',
    },
  )
 ...
 * 온보딩 정보 저장 응답 타입
 */
export type SaveOnboardingResponse = Record<string, never>
...
export const saveOnboarding = async (
  ...
  const response = await authenticatedFetch(
    `${API_BASE_URL}/account/onboarding`,
    {
      method: 'POST',
      body: JSON.stringify(request),
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `온보딩 정보 저장 실패: ${response.status} ${response.statusText}`,
    )
  }
