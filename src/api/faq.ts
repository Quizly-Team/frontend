...
export const getFaqs = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `FAQ 조회 실패: ${response.status} ${response.statusText}`,
    )
  }
...
export const createAdminFaq = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `FAQ 등록 실패: ${response.status} ${response.statusText}`,
    )
  }
...
export const deleteAdminFaq = async (faqId: number): Promise<void> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message ||
        `FAQ 삭제 실패: ${response.status} ${response.statusText}`,
    )
  }
}
