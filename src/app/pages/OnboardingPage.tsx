...
const OnboardingPage = () => {
  ...
  const [, setStep1Completed] = useState(false)
  ...
  const userTypes = [
    {
      ...
      label: '고등학생',
      character: '/characters/character1.png',
    },
    {
      id: 'university' as const,
      label: '대학(원)생',
      character: '/characters/character2.png',
    },
    {
      id: 'jobseeker' as const,
      label: '취준생',
      character: '/characters/character3.png',
    },
    {
      id: 'general' as const,
      label: '일반 학습자',
      character: '/characters/character4.png',
    },
  ]
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      setIsLoading(false)
      alert(
        `온보딩 정보 저장에 실패했습니다: ${error instanceof Error ? error.message : '알 수 없는 오류'}`,
      )
    }
        ...
        <div className="flex flex-col items-center max-md:pt-[20px] md:pt-[60px] max-md:pb-[106px] max-md:h-full w-full max-w-[700px] md:pb-8">
          ...
          <div className="flex items-center justify-center mb-[47px] max-md:mb-[20px] max-md:shrink-0">
              ...
              <div className="relative flex items-center justify-center w-[46px] h-[46px] max-md:w-[32px] max-md:h-[32px]">
                {step > 1 ? (
                  ...
                  <div className="w-[46px] h-[46px] max-md:w-[32px] max-md:h-[32px] rounded-full bg-white border border-[#dedede] flex items-center justify-center">
                    <span className="text-[20px] max-md:text-[16px] font-medium text-[#b7b7b7]">
                      1
                    </span>
                  </div>
                )}
            ...
            >
              ...
              <span
                className={`text-[16px] max-md:text-[14px] font-medium max-md:font-normal whitespace-nowrap ${
                  step === 2 || selectedGoals.length > 0 || selectedUserType
                    ? 'text-primary'
                    : 'text-[#b7b7b7]'
                }`}
              >
          ...
          {/* Content Box */}
          <div
            className={`bg-white rounded-[24px] border border-[#dedede] w-full max-w-[700px] max-md:max-w-[335px] max-md:w-[335px] max-md:mx-auto relative flex flex-col items-center max-md:shrink-0 ${step === 1 ? 'max-md:h-[364px]' : 'max-md:h-[492px]'}`}
          >
            {/* Step 1: User Type Selection */}
            {step === 1 && (
              <>
                <div className="flex items-center justify-center w-full max-md:mt-[30px] md:mt-[60px] px-4">
                  <h1
                  ...
                  {userTypes.map((type) => (
                    ...
                    >
                      <img
                        ...
                        className={
                          type.id === 'university'
                            ? 'w-[57px] h-[52px] max-md:w-[40px] max-md:h-[36px]'
                            : 'w-[52px] h-[52px] max-md:w-[36px] max-md:h-[36px]'
                        }
                      />
            ...
            {step === 2 && (
              <>
                <div className="flex items-center justify-center w-full max-md:mt-[30px] md:mt-[60px]">
                  <h1
                  ...
                  {learningGoals.map((goal) => {
                    ...
                    return (
                      ...
                      >
                        <Icon
                          name={
                            isSelected ? 'icn_check_fill_in' : 'icn_check_fill'
                          }
                          size={28}
                          className="shrink-0"
                        />
          ...
          {step === 1 && (
            <div
              className="hidden max-md:block bg-white max-md:shrink-0 max-md:fixed max-md:bottom-0 max-md:left-0 max-md:right-0 max-md:w-full"
              style={{
                height: '106px',
                maxWidth: '375px',
                margin: '0 auto',
