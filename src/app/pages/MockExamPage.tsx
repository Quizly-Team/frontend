...
const MockExamPage = () => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- mockExamList는 매 렌더 새 배열이 될 수 있어 useMemo로 감싸면 effect 발화 시점이 달라짐. 런타임 동작 유지를 위해 로직 변경 없이 경고만 억제.
    ...
    try {
      ...
      const canvas = await html2canvas(clonedElement, {
        ...
        onclone: (clonedDoc) => {
          ...
          const optionNumbers = clonedDoc.querySelectorAll(
            '[data-option-number]',
          )
  ...
  return (
    <div className="flex-1 bg-bg-home max-md:pb-20">
      ...
      <div
        ...
        style={{
          paddingLeft: `${currentMargin}px`,
          paddingRight: `${currentMargin}px`,
        }}
      >
                      ...
                      {mockExamList
                        ...
                        .map((question, index) => (
                          <MockExamQuestion
                            ...
                            question={question}
                            questionNumber={
                              Math.ceil(mockExamList.length / 2) + index + 1
                            }
                            showAnswer={false}
                          />
                        ))}
