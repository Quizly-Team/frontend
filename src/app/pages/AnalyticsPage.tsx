...
const AnalyticsPage = () => {
  ...
  // 미리보기 이미지 URL 정리
  // eslint-disable-next-line react-hooks/rules-of-hooks -- 위 조건부 early return(!isAuthenticated) 때문에 훅이 조건부 호출로 감지됨. 훅 위치를 옮기면 실행 시점이 바뀌어 런타임 동작이 달라지므로 동작 보존을 위해 규칙만 무시함.
  useEffect(() => {
    return () => {
      if (previewImageUrl) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setSaveError(
        err instanceof Error ? err.message : '변경사항 저장에 실패했습니다.',
      )
    } finally {
      setIsSaving(false)
    }
                ...
                <>
                    ...
                    <div className="overflow-x-auto -mx-5 px-5">
                      <div className="flex gap-[20px] min-w-max pb-2">
                        <div className="flex-shrink-0 w-[250px]">
                          <TodaySummary
                            data={dashboardData.todaySummary}
                            dailyData={dashboardData.dailySummaryList}
                          />
                        </div>
                        <div className="flex-shrink-0 w-[250px]">
                          <CumulativeSummary
                            data={dashboardData.cumulativeSummary}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-5 max-md:flex-col max-md:gap-[20px]">
                    ...
                    <div className="min-w-0 flex-1 max-w-[478px] max-lg:max-w-[442px] max-md:hidden">
                      <HourlyChart
                        data={dashboardData.hourlySummaryList}
