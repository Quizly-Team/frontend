...
const MockExamQuestion = ({
          ...
          <div className="space-y-2">
            {question.options.map((option, index) => (
              <div key={index} className="flex items-start gap-[6px]">
                <div
                  className="relative flex-shrink-0 w-[18px] h-[18px] mt-[2px]"
                  data-option-number
                >
            ...
            <div className="mb-3 p-4 border border-[#222222] rounded-md bg-white relative">
              ...
              <div className="space-y-3">
                {viewOptions.map((option, index) => (
                  <p
                    key={index}
                    className="text-base font-normal text-[#222222] leading-[1.4]"
                  >
                    {option}
                  </p>
                ))}
  ...
  const renderAnswer = () => {
    ...
    return (
      <div className="space-y-2">
        <p className="text-base font-medium text-primary">해설 요약</p>
  ...
  if (showAnswer) {
    ...
    const renderCircleNumber = (num: number) => {
      return (
        <div
          className="relative inline-flex items-center justify-center w-[18px] h-[18px] align-middle"
          style={{ marginTop: '-2px' }}
        >
          <svg
            width="18"
      ...
      if (question.type === 'FIND_MATCH') {
        if (question.options.length > 0) {
          ...
          if (circleMatch) {
            ...
            const restOfAnswer = question.answer
              .replace(/^[①②③④⑤]\s*/, '')
              .trim()
            return (
  ...
  // 문제 모드: 전체 문제 표시
  return <div className="mb-5">{renderQuestionContent()}</div>
}
